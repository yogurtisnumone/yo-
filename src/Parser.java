...
public class Parser {//OK
    ...
    private Statement parseStatement() {
        System.out.println("Parsing statement, next token: " + tokenizer.peekToken());
    ...
    private Expression parseExpression() {
        ...
        while (tokenizer.hasNext()) {
            ...
            if (!isComparisonOperator(tokenizer.peekToken())) break;
            tokenizer.nextToken();
            Expression right = parseSum();
            left = new CompareExpression(left, right, operator);
        }
        return left;
    }
    ...
    private Expression parsePower() {
        ...
        if (token.matches("\\d+")) {
            ...
            return switch (token) {
                ...
                case "random" -> new RandomExpression();
